...
class CupcakeMigrationTest {
    ...
    fun `migration 7 to latest preserves user data and passes Room validation`() = runBlocking {
        ...
        val book = reminders.first { it.type == ReminderType.BOOK && it.title == "Saga" }
        ...
        // v10 goal-model columns get safe defaults on resume-mode rows.
    ...
    private fun createV7Database(file: File) {
        ...
        try {
            ...
            db.execSQL(
                "INSERT INTO reminders VALUES " +
                    "(3, 1, 'BOOK', 'DAILY', 8, 1, 1, 'LOCAL', 'Daily Page', NULL, " +
                    "0, '/books/page.epub.cbz', 0, NULL, 'PAGE_A_DAY', 5, 3, 0, 0)",
            )
            db.version = 7
        } finally {
            db.close()
